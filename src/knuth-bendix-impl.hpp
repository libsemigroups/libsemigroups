...
namespace libsemigroups {
  ...
  namespace fpsemigroup {
    class KnuthBendix::KnuthBendixImpl {
      ...
      friend class Rule;  // defined in knuth-bendix/rule-impl.hpp
     ...
     private:
      ...
      void add_rule(Rule* rule) {
        ...
        if (rule->lhs()->size() < _min_length_lhs_rule) {
          // FIXME(later) this is not valid when using non-length reducing
     ...
     public:
      ...
      ReductionOrdering const* order() const {
        return _order;
      }
     ...
     private:
      ...
      // FIXME(now) there is a possibly infinite loop here clear_stack ->
