...
namespace libsemigroups {
  ...
  //! Returns \c true if the given permutaion is an element of the permutation
  //! group generated by the given vector of permutations, and \c false
  //! otherwise.
  template <typename PointType>
  extern uint64_t group_mem(std::vector<Permutation<PointType>*>* const gens,
                            Permutation<PointType>* const               test) {
    uint64_t               deg  = ((*gens)[0])->degree();
    std::vector<PointType> base = {};
    std::vector<uint64_t>  size_orbits(deg);
    std::vector<PointType> orbits(deg * deg);
    ...
    for (uint64_t i = 0; i < base.size(); i++) {
      if ((*transversal_inv)[i * deg + (*currenttest)[base[i]]] == nullptr) {
        out = false;
        break;
      } else if (currenttest->is_identity()) {
        break;
      }
      currenttest_copy = currenttest->really_copy();
      currenttest->redefine(
          currenttest_copy,
          (*transversal_inv)[i * deg + (*currenttest)[base[i]]]);
      currenttest_copy->really_delete();
      delete currenttest_copy;
    }
    free_stab_chain<PointType>(transversal, transversal_inv, strong_gens, deg);
    out = out && currenttest->is_identity();
    currenttest->really_delete();
    delete currenttest;
    return out;
  }

  //! Returns \c true if the stabiliser of the given point, in the permutaion
  //! group generated by the first vector of permutations, is trivial and
  //! \c false otherwise.
  //!
  //! Also changes the second vector of permutations into a generating set for
  //! this stabilizer.
  template <typename PointType>
  extern bool point_stabilizer(std::vector<Permutation<PointType>*>* const gens,
                               PointType const                             pt,
                               std::vector<Permutation<PointType>*>* out) {
    uint64_t               deg  = ((*gens)[0])->degree();
    std::vector<PointType> base = {};
    ...
    add_base_point<PointType>(
        pt, &base, &orbits, &size_orbits, transversal, transversal_inv, deg);
    schreier_sims_stab_chain<PointType>(0,
                                        &base,
                                        &orbits,
                                        &size_orbits,
                                        ...
                                        deg);
    if (out != nullptr)
      really_delete_cont<std::vector<Permutation<PointType>*>, PointType>(out);

    if ((*strong_gens)[1] == nullptr) {
      // this means that the stabilizer of pt under <gens> is trivial
      *out = {
          static_cast<Permutation<PointType>*>((*transversal)[0]->identity())};
      free_stab_chain<PointType>(
          transversal, transversal_inv, strong_gens, deg);
      return true;
    }
    std::vector<Permutation<PointType>*>* out_temp
        = really_copy_vec<PointType>((*strong_gens)[1]);
    *out = *out_temp;
    delete out_temp;
    free_stab_chain<PointType>(transversal, transversal_inv, strong_gens, deg);
    return false;
