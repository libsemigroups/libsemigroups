...
//! Limit of thread cache in number of spans for each page count class (undefine
//! for unlimited cache - i.e never release spans to global cache unless thread
//! finishes)
...
#define MIN_SPAN_CACHE_RELEASE 16
//! Maximum cache size divisor (max cache size will be max allocation count
//! divided by this divisor)
#define MAX_SPAN_CACHE_DIVISOR 8
//! Multiplier for global span cache limit (max cache size will be calculated
//! like thread cache and multiplied with this)
...
#define ALIGNED_STRUCT(name, alignment) \
  struct __attribute__((__aligned__(alignment))) name
...
#define atomic_thread_fence_release()  //__asm volatile("" ::: "memory")
    ...
    || defined(_AMD64_) || defined(__arm64__) || defined(__aarch64__)
#define ARCH_64BIT 1
#else
#define ARCH_64BIT 0
#endif
...
static FORCEINLINE int64_t atomic_exchange_and_add64(atomic64_t* dst,
                                                     int64_t     add) {
...
#ifdef _MSC_VER
  int32_t old
      = (int32_t) _InterlockedExchangeAdd((volatile long*) &val->nonatomic, 1);
  return (old + 1);
#else
...
#ifdef _MSC_VER
  int32_t old = (int32_t) _InterlockedExchangeAdd(
      (volatile long*) &val->nonatomic, add);
  return (old + add);
#else
...
#define SMALL_CLASS_COUNT \
  (((PAGE_SIZE - SPAN_HEADER_SIZE) >> 1) >> SMALL_GRANULARITY_SHIFT)
...
struct heap_t {
  ...
  //! List of demi-used spans with free blocks for each size class (double
  //! linked list)
...
static void* _memory_map(size_t page_count);

static void _memory_unmap(void* ptr, size_t page_count);

static int _memory_deallocate_deferred(heap_t* heap, size_t size_class);

//! Lookup a memory heap from heap ID
static heap_t* _memory_heap_lookup(int32_t id) {
...
static size_t _span_class_from_page_count(size_t page_count) {
  assert((page_count > 0) && (page_count <= 16));
  return ((page_count + SPAN_CLASS_GRANULARITY - 1) / SPAN_CLASS_GRANULARITY)
         - 1;
}

//! Increase an allocation counter
static void _memory_counter_increase(span_counter_t* counter,
...
//! Insert the given list of memory page spans in the global cache for
//! small/medium blocks
static void _memory_global_cache_insert(span_t* first_span,
                                        size_t  list_size,
                                        size_t  page_count) {
  ...
  while (1) {
    ...
    void*  global_span_ptr
        = atomic_load_ptr(&_memory_span_cache[span_class_idx]);
    if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
      uintptr_t global_list_size = (uintptr_t) global_span_ptr & ~SPAN_MASK;
      span_t*   global_span
          = (span_t*) ((void*) ((uintptr_t) global_span_ptr & SPAN_MASK));

#ifdef GLOBAL_SPAN_CACHE_MULTIPLIER
      size_t cache_limit
          = GLOBAL_SPAN_CACHE_MULTIPLIER
            * (_memory_max_allocation[span_class_idx] / MAX_SPAN_CACHE_DIVISOR);
      if ((global_list_size >= cache_limit)
          && (global_list_size > MIN_SPAN_CACHE_SIZE))
        break;
#endif
      ...
      void* first_span_ptr
          = (void*) ((uintptr_t) first_span | global_list_size);
      if (atomic_cas_ptr(&_memory_span_cache[span_class_idx],
                         first_span_ptr,
                         global_span_ptr))
        return;
    } else {
...
//! Extract a number of memory page spans from the global cache for small/medium
//! blocks
static span_t* _memory_global_cache_extract(size_t page_count) {
  span_t*      span           = 0;
  size_t       span_class_idx = _span_class_from_page_count(page_count);
  ...
  while (global_span_ptr) {
    if ((global_span_ptr != SPAN_LIST_LOCK_TOKEN)
      ...
      // stored in prev_span to quickly skip ahead in the list to get the new
      // head
...
/*! Insert the given list of memory page spans in the global cache for large
   blocks,
    similar to _memory_global_cache_insert */
static void _memory_global_cache_large_insert(span_t* span_list,
                                              size_t  list_size,
                                              size_t  span_count) {
  ...
  while (1) {
    ...
    if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
      ...
      size_t cache_limit = GLOBAL_SPAN_CACHE_MULTIPLIER
                           * (_memory_max_allocation_large[span_count - 1]
      ...
      global_list_size += list_size;
      void* new_global_span_ptr
          = (void*) ((uintptr_t) span_list | global_list_size);
...
/*! Extract a number of memory page spans from the global cache for large
   blocks,
    similar to _memory_global_cache_extract */
static span_t* _memory_global_cache_large_extract(size_t span_count) {
  span_t*      span  = 0;
  atomicptr_t* cache = &_memory_large_cache[span_count - 1];
  atomic_thread_fence_acquire();
  void* global_span_ptr = atomic_load_ptr(cache);
...
static void* _memory_allocate_from_heap(heap_t* heap, size_t size) {
#if ENABLE_STATISTICS
      ...
      = _memory_size_class
            [(size <= SMALL_SIZE_LIMIT)
                 ? ((size + (SMALL_GRANULARITY - 1)) >> SMALL_GRANULARITY_SHIFT)
                       - 1
                 : SMALL_CLASS_COUNT
                       + ((size - SMALL_SIZE_LIMIT + (MEDIUM_GRANULARITY - 1))
...
// Step 1: Try to get a block from the currently active span. The span block
// bookkeeping
...
use_active:
  ...
  // Step 2: No active span, try executing deferred deallocations and try again
  // if there
  ...
  // Step 3: Check if there is a semi-used span of the requested size class
  // available
  if (heap->size_cache[class_idx]) {
    // Promote a pending semi-used span to be active, storing bookkeeping data
    // in
  ...
  if (!span) {
    // Step 5: No span available in the thread cache, try grab a list of spans
    // from the global cache
    span = _memory_global_cache_extract(size_class->page_count);
#if ENABLE_STATISTICS
    if (span)
      heap->global_to_thread
          += (size_t) span->data.list_size * size_class->page_count * PAGE_SIZE;
#endif
  }
  if (span) {
    if (span->data.list_size > 1) {
      // We got a list of spans, we will use first as active and store remainder
      // in thread cache
      span_t* next_span = span->next_span;
      assert(next_span);
...
#if ENABLE_STATISTICS
  // Store the requested size for statistics
  *(size_t*) pointer_offset(span,
                            SPAN_HEADER_SIZE + class_size - sizeof(size_t))
      = size;
#endif
...
static void* _memory_allocate_large_from_heap(heap_t* heap, size_t size) {
  ...
  if (!idx) {
    ...
    if (!span) {
      ...
      if (span)
        heap->global_to_thread
            += (size_t) span->data.list_size * SPAN_MAX_PAGE_COUNT * PAGE_SIZE;
#endif
    }
    if (span) {
      if (span->data.list_size > 1) {
        // We got a list of spans, we will use first as active and store
        // remainder in thread cache
        span_t* next_span = span->next_span;
        assert(next_span);
        next_span->data.list_size        = span->data.list_size - 1;
        heap->span_cache[span_class_idx] = next_span;
...
use_cache:
  // Step 1: Check if cache for this large size class (or the following, unless
  // first class) has a span
  while (!heap->large_cache[idx] && (idx < LARGE_CLASS_COUNT)
         && (idx < num_spans + 1))
    ++idx;
  ...
  if (span) {
    ...
    // Increase counter
    _memory_counter_increase(&heap->large_counter[idx],
                             &_memory_max_allocation_large[idx]);
...
#if ENABLE_STATISTICS
    heap->global_to_thread
        += (size_t) span->data.list_size * num_spans * SPAN_MAX_SIZE;
#endif
...
//! Allocate a new heap
static heap_t* _memory_allocate_heap(void) {
  heap_t* heap;
  heap_t* next_heap;
...
static void _memory_list_add(span_t** head, span_t* span) {
  if (*head) {
    (*head)->prev_span = span;
    span->next_span    = *head;
  } else {
...
static void _memory_list_remove(span_t** head, span_t* span) {
  if (*head == span) {
    *head = span->next_span;
  } else {
    if (span->next_span)
...
_memory_heap_cache_insert(heap_t* heap, span_t* span, size_t page_count) {
  ...
  *cache                 = span;
  ...
  if ((span->data.list_size >= (MIN_SPAN_CACHE_RELEASE + MIN_SPAN_CACHE_SIZE))
      && (span->data.list_size
          > heap->span_counter[span_class_idx].cache_limit)) {
...
static void _memory_deallocate_to_heap(heap_t* heap, span_t* span, void* p) {
  ...
  span_block_t* block_data
      = is_active ? heap->active_block + class_idx : &span->data.block;

#if ENABLE_STATISTICS
  heap->allocated -= size_class->size;
  heap->requested
      -= *(size_t*) pointer_offset(p, size_class->size - sizeof(size_t));
#endif
  ...
  if (block_data->free_count == ((count_t) size_class->block_count - 1)) {
    ...
    // partial free list if we had a previous free block (guard for classes with
    // only 1 block)
    if (is_active)
  ...
  // Span is not yet completely free, so add block to the linked list of free
  // blocks
...
static void _memory_deallocate_large_to_heap(heap_t* heap, span_t* span) {
...
static void _memory_deallocate_defer(int32_t heap_id, void* p) {
...
static void* _memory_allocate(size_t size) {
...
static size_t _memory_usable_size(void* p) {
  // Grab the span (always at start of span, using 64KiB alignment)
  span_t* span    = (void*) ((uintptr_t) p & SPAN_MASK);
  int32_t heap_id = atomic_load32(&span->heap_id);
  if (heap_id) {
...
static void _memory_adjust_size_class(size_t iclass) {
  ...
  page_count
      = ((page_count + (SPAN_CLASS_GRANULARITY - 1)) / SPAN_CLASS_GRANULARITY)
        * SPAN_CLASS_GRANULARITY;
  if (page_count > 16)
    page_count = 16;
  size_t block_count
      = ((page_count * PAGE_SIZE) - SPAN_HEADER_SIZE) / block_size;
  ...
  while (prevclass > 0) {
    ...
    if ((_memory_size_class[prevclass].page_count
         == _memory_size_class[iclass].page_count)
        && (_memory_size_class[prevclass].block_count
    ...
    while (span) {
      if (span->data.list_size > MIN_SPAN_CACHE_RELEASE) {
        ...
        _memory_global_cache_large_insert(
            span, span->data.list_size, span_count);
        span = 0;
      }
    }
...
static void* _memory_map(size_t page_count) {
...
#ifdef PLATFORM_WINDOWS
  pages_ptr
      = VirtualAlloc(0, total_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
#else
  ...
  if (total_size % SPAN_ADDRESS_GRANULARITY)
    ++incr;
  do {
    void* base_addr = (void*) (uintptr_t) atomic_exchange_and_add64(
        &_memory_addr, (incr * (intptr_t) SPAN_ADDRESS_GRANULARITY));
    pages_ptr = mmap(base_addr,
                     total_size,
...
static FORCEINLINE int atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) {
  ...
  return (_InterlockedCompareExchange(
              (volatile long*) &dst->nonatomic, (long) val, (long) ref)
          == (long) ref)
             ? 1
             : 0;
#endif
...
void* rpaligned_realloc(void*        ptr,
                        size_t       alignment,
                        size_t       size,
                        size_t       oldsize,
                        unsigned int flags) {
...
void* rpaligned_alloc(size_t alignment, size_t size) {
  ...
  if ((uintptr_t) ptr & (alignment - 1))
    ptr = (void*) (((uintptr_t) ptr & ~((uintptr_t) alignment - 1))
                   + alignment);
  return ptr;
}
...
void rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats) {
  ...
  for (size_t isize = 0; isize < SIZE_CLASS_COUNT; ++isize) {
    if (heap->active_block[isize].free_count)
      stats->active
          += heap->active_block[isize].free_count
             * _memory_size_class[heap->active_span[isize]->size_class].size;

    span_t* cache = heap->size_cache[isize];
    while (cache) {
      stats->sizecache = cache->data.block.free_count
                         * _memory_size_class[cache->size_class].size;
  ...
  for (size_t isize = 0; isize < SPAN_CLASS_COUNT; ++isize) {
    if (heap->span_cache[isize])
      stats->spancache = (size_t) heap->span_cache[isize]->data.list_size
                         * (isize + 1) * SPAN_CLASS_GRANULARITY * PAGE_SIZE;
  }
}
...
void rpmalloc_global_statistics(rpmalloc_global_statistics_t* stats) {
  ...
  for (size_t iclass = 0; iclass < SPAN_CLASS_COUNT; ++iclass) {
    ...
    uintptr_t global_span_count = (uintptr_t) global_span_ptr & ~SPAN_MASK;
    size_t    list_bytes
        = global_span_count * (iclass + 1) * SPAN_CLASS_GRANULARITY * PAGE_SIZE;
  ...
  for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
    ...
    uintptr_t global_span_count = (uintptr_t) global_span_ptr & ~SPAN_MASK;
    size_t    list_bytes
        = global_span_count * (iclass + 1) * SPAN_MAX_PAGE_COUNT * PAGE_SIZE;
