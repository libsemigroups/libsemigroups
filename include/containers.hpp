  ...
  namespace internal {
    ...
    class RecVec {
     ...
     public:
      ...
      // Adds the specified number of rows to the <RecVec>. Every new position
      ...
      // Adds the specified number of columns to the <RecVec>. Every new
      // position is initialised with the default value specified when the
      // <RecVec> was constructed.
      //
      ...
      void add_cols(size_t nr) {
        ...
        size_t new_nr_cols
            = std::max(5 * old_nr_cols / 4 + 4, nr + old_nr_cols);
        if (_nr_rows != 0) {
      ...
      //
      // Asserts whether the specified row and column are within the bounds of
      // the <RecVec>.
      ...
      class iterator_base {
       ...
       private:
        ...
        struct fast_iterator_methods : public iterator_methods {
          ...
          inline void
          prefix_increment(iterator_base& it) const final {  // prefix
            ++it._it_vec;
          }
        ...
        struct slow_iterator_methods : public iterator_methods {
          ...
          inline void increment(iterator_base& it,
                                size_type      val) const override {
          ...
          inline void decrement(iterator_base& it,
                                size_type      val) const override {
            size_type s = _rv._nr_used_cols;
            size_type n = _rv._nr_unused_cols;
     ...
     public:
      ...
      iterator begin_row(size_t row_index) {
        ...
        return iterator(*this,
                        _vec.begin()
                            + (_nr_used_cols + _nr_unused_cols) * row_index);
      }
      ...
      const_iterator cbegin_row(size_t row_index) const {
        ...
        return const_iterator(
            *this,
            _vec.begin() + (_nr_used_cols + _nr_unused_cols) * row_index);
      }
    ...
    typename RecVec<T, A>::template iterator_base<
        iterator_type,
        reference_type,
        pointer_type>::iterator_methods const*
