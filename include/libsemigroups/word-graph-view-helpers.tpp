...
namespace libsemigroups {
  ...
  namespace word_graph {
    ...
    bool is_reachable_no_checks(WordGraphView<Node1> const& wgv,
                                Node2                       source,
                                Node2                       target) {
    ...
    template <typename Node1, typename Node2>
    bool is_reachable(WordGraphView<Node1> const& wgv,
                      Node2                       source,
                      Node2                       target) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
    ...
    template <typename Node1, typename Node2>
    bool is_acyclic(WordGraphView<Node1> const& wgv, Node2 source) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
      ::libsemigroups::detail::throw_if_not_less(
          source, wgv.number_of_nodes_no_checks(), "node ");
      auto const        N = wgv.number_of_nodes_no_checks();
    ...
    template <typename Node1, typename Node2>
    bool is_acyclic(WordGraphView<Node1> const& wgv,
                    Node2                       source,
                    Node2                       target) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
      ::libsemigroups::detail::throw_if_not_less(
      ...
      for (auto m : wgv.nodes_no_checks()) {
        if (seen[m] == 0) {
          ...
          if (!detail::topological_sort(wgv, stck, seen, order)) {
            // graph is not acyclic and so there's no topological order
            // for the nodes.
    ...
    template <typename Node1, typename Node2>
    std::vector<Node1> topological_sort(WordGraphView<Node1> const& wgv,
                                        Node2                       source) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
        ...
        if (seen.insert(s).second && depth < max_depth) {
          for (auto t : wgv.targets_no_checks(s)) {
            if (t < N) {
              ...
              // accessed again, so it doesn't matter that it is wrong.
              // O/w if "t" has not been seen before the "depth + 1" is
              // its correct depth.
    ...
    ancestors_of_no_checks(WordGraphView<Node1> const& wgv, Node2 target) {
      ...
      // target in the reversed graph. Since the reverse of a WordGraph is
      // no longer a WordGraph we use a vector of vectors here.
      // Alternatively, we could use the technique used in
      // WordGraphWithSources (the sources are essentially the reversed
      // graph) to create the reversed graph (or just use it if we know it
      // already, like in ToddCoxeter).
    ...
    template <typename Node1, typename Node2>
    std::unordered_set<Node1> ancestors_of(WordGraphView<Node1> const& wgv,
                                           Node2                       target) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
      ::libsemigroups::detail::throw_if_not_less(
          target, wgv.number_of_nodes_no_checks(), "node ");
    ...
    Node1 follow_path_no_checks(WordGraphView<Node1> const& wgv,
                                Node2                       source,
                                Iterator                    first,
    ...
    template <typename Node1, typename Node2, typename Iterator>
    Node1 follow_path(WordGraphView<Node1> const& wgv,
                      Node2                       source,
                      Iterator                    first,
                      Iterator                    last) {
      static_assert(sizeof(Node1) <= sizeof(size_t));
    ...
    Dot dot(WordGraphView<Node> const&      wgv,
      ...
      if (node_labels.size() != wgv.number_of_nodes_no_checks()) {
        LIBSEMIGROUPS_EXCEPTION("expected the 2nd argument (node labels) "
                                "to have size {}, the "
                                "number of nodes of the 1st argument "
                                "(word graph), but found {}",
                                wgv.number_of_nodes_no_checks(),
                                node_labels.size());
      } else if (edge_labels.size() != wgv.out_degree()) {
        LIBSEMIGROUPS_EXCEPTION(
            "expected the 3rd argument (edge labels) to have size {}, "
            "the "
      ...
      for (size_t index = 0; index < out_degree; ++index) {
        label += fmt::format("<tr><td align=\"right\" "
                             "port=\"port{}\">{}&nbsp;</td></tr>\n",
                             index,
                             edge_labels[index]);
      }
    ...
    template <typename Node1, typename Node2>
    void spanning_tree_no_checks(WordGraphView<Node1> const& wgv,
                                 Node2                       root,
                                 Forest&                     f,
                                 size_t                      max_depth) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
    ...
    template <typename Node1, typename Node2>
    void spanning_tree(WordGraphView<Node1> const& wgv,
                       Node2                       root,
                       Forest&                     f,
                       size_t                      max_depth) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
    ...
    template <typename Node1, typename Node2>
    Forest spanning_tree(WordGraphView<Node1> const& wgv,
                         Node2                       root,
                         size_t                      max_depth) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
      Forest f;
    ...
    template <typename Node1, typename Node2>
    Forest spanning_tree_no_checks(WordGraphView<Node1> const& wgv,
                                   Node2                       root,
                                   size_t                      max_depth) {
      static_assert(sizeof(Node2) <= sizeof(Node1));
      Forest f;
