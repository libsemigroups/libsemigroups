...
template <typename W> class Presentation : public PresentationBase {
public:
...
public:
  ...
  //!
  //! \param (None)
  word_type const &alphabet() const noexcept { return _alphabet; }
  ...
  //! add_rule
  template <typename S, typename T>
  Presentation &add_rule_and_check(S lhs_begin, S lhs_end, T rhs_begin,
                                   T rhs_end) {
  ...
  //! \exceptions
  //! \noexcept
  bool contains_empty_word() const noexcept { return _contains_empty_word; }
  ...
  //! \returns
  //! (None)
  template <typename T> void validate_word(T first, T last) const;
...
//! No checks that the arguments describe words over the alphabet of the
//! presentation are performed.
inline void add_rule(Presentation<std::string> &p, char const *lhop,
                     char const *rhop) {
...
//! letters not belonging to `p.alphabet()`.
template <typename W>
void add_rule_and_check(Presentation<W> &p, char const *lhop,
                        char const *rhop) {
...
void add_rule(Presentation<W> &p, std::initializer_list<T> lhop,
...
//! letters not belonging to `p.alphabet()`.
template <typename W, typename T>
void add_rule_and_check(Presentation<W> &p, std::initializer_list<T> lhop,
                        std::initializer_list<T> rhop) {
  p.add_rule_and_check(lhop.begin(), lhop.end(), rhop.begin(), rhop.end());
}
...
template <typename W>
void add_inverse_rules(Presentation<W> &p, W const &vals,
...
//! \complexity
//! \f$O(n)\f$ where \f$n\f$ is alphabet().size().
inline void add_inverse_rules(Presentation<std::string> &p, char const *vals,
                              char e = UNDEFINED) {
...
template <typename W> void remove_duplicate_rules(Presentation<W> &p);
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> void remove_trivial_rules(Presentation<W> &p);
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> void reduce_complements(Presentation<W> &p);
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> void sort_each_rule(Presentation<W> &p);
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> void sort_rules(Presentation<W> &p);
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> bool are_rules_sorted(Presentation<W> const &p);
...
template <typename W> W longest_common_subword(Presentation<W> &p);
...
// TODO(later) complexity
template <typename W> void replace_subword(Presentation<W> &p, W const &w) {
...
// TODO(later) complexity
template <typename W>
void replace_subword(Presentation<W> &p, W const &existing,
                     W const &replacement);
...
void replace_subword(Presentation<W> &p, S first_existing, S last_existing,
                     T first_replacement, T last_replacement);
...
//! \no_libsemigroups_except
template <typename W>
void replace_word(Presentation<W> &p, W const &existing, W const &replacement);
...
//! \exceptions
//! \no_libsemigroups_except
template <typename W> size_t length(Presentation<W> const &p) {
...
//! \exceptions
//! \no_libsemigroups_except
template <typename W> void reverse(Presentation<W> &p) {
...
//! presentation.
template <typename W> void normalize_alphabet(Presentation<W> &p);
...
//! last is odd.
template <typename T> T longest_rule(T first, T last);
...
template <typename W> auto longest_rule(Presentation<W> const &p) {
...
//! \throws LibsemigroupsException if the distance between \p first and \p
//! last is odd.
template <typename T> T shortest_rule(T first, T last);
...
//!
//! \throws LibsemigroupsException if the length of \p p.rules is odd.
template <typename W> auto shortest_rule(Presentation<W> const &p) {
...
//!
//! \throws LibsemigroupsException if the length of \p p.rules is odd.
template <typename T> auto longest_rule_length(T first, T last);
...
//!
//! \throws LibsemigroupsException if the length of \p p.rules is odd.
template <typename W> auto longest_rule_length(Presentation<W> const &p) {
...
//!
//! \throws LibsemigroupsException if the length of \p p.rules is odd.
template <typename T> auto shortest_rule_length(T first, T last);
...
//!
//! \throws LibsemigroupsException if the length of \p p.rules is odd.
template <typename W> auto shortest_rule_length(Presentation<W> const &p) {
...
//!
//! \throws LibsemigroupsException if `p.rules.size()` is odd.
template <typename W> void remove_redundant_generators(Presentation<W> &p);
...
template <typename W>
typename Presentation<W>::letter_type letter(Presentation<W> const &, size_t i);
...
//!  `replace_word` does.
template <typename W> void greedy_reduce_length(Presentation<W> &p);
...
template <typename T, typename = std::enable_if_t<std::is_same_v<T, word_type>>>
