...
// are two types of node, those that are "num_nodes_active" and those that are
// "free".
//
// If c is a node, then
...
//
// If c is an num_nodes_active node, then _ident[c] = c.
//
...
//     _current always points to an num_nodes_active node, even if the value
//     changes during a function call.
//
//   * _current_la: is similar to _current, and can be used independently of
//     _current.
//
//   * _last_active_node points to the final num_nodes_active node in the list.
//
...
namespace libsemigroups {
  namespace detail {
    ...
    void NodeManager<Node>::switch_nodes(Node c, Node d) {
      ...
      // everything is finished and so _current may not be num_nodes_active.

      LIBSEMIGROUPS_ASSERT(is_active_node(_last_active_node));
      LIBSEMIGROUPS_ASSERT(!is_active_node(_first_free_node));
    }

    // The permutation p ^ -1 must map the num_nodes_active nodes to the [0, ..
    ...
    void NodeManager<Node>::free_node(Node c) {
      ...
      if (c == _last_active_node) {
        ...
        // Remove <c> from the num_nodes_active list
