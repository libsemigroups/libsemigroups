...
namespace libsemigroups {
  ...
  // KnuthBendixImpl can be reversed if necessary. This wouldn't be necessary if
  // we knew at compile time that a KnuthBendixImpl object represented a left
  // congruence, since we could just return something like "Paths |
  // transform(reverse)". But since the handedness of KnuthBendixImpl can be set
  // at runtime we require a single return type (i.e. can't conditionally return
  // "Paths" if right or 2-sided, and "Paths | transform(reverse)" if left,
  // because these are different types) for KnuthBendixImpl::normal_forms,
