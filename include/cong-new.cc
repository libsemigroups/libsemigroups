...
namespace libsemigroups {
  ...
  Congruence::Congruence(congruence_t                   type,
      ...
      : Congruence(type) {
    _race.add_runner(
        new ToddCoxeter(type, S, genpairs, ToddCoxeter::policy::use_relations));
    _race.add_runner(new ToddCoxeter(
        type, S, genpairs, ToddCoxeter::policy::use_cayley_graph));
  }

  Congruence::Congruence(congruence_t                      type,
                         SemigroupBase*                    S,
                         std::initializer_list<relation_t> extra)
      : Congruence(type, S, std::vector<relation_t>(extra)) {}

  Congruence::Congruence(congruence_t                   type,
                         FpSemigroup&                   S,
                         std::vector<relation_t> const& genpairs)
      : Congruence(type, &S, genpairs) {}
  ...
  Congruence::Congruence(congruence_t                   type,
      ...
      : Congruence(type) {
    // FIXME revise this!
    _race.set_max_threads(INFTY);
    if (S->todd_coxeter() != nullptr) {
      ...
      // structure for S.
      _race.add_runner(new ToddCoxeter(S->todd_coxeter(), genpairs));
    ...
    if (S->rws() != nullptr) {
      if (S->rws()->has_isomorphic_non_fp_semigroup()
          && S->rws()->isomorphic_non_fp_semigroup()->is_done()) {
        // Method 3: Note that the S->rws()->isomorphic_non_fp_semigroup()
        // must be finite in this case, because otherwise it would not return
        // true from Semigroup::is_done. This is similar to Method 2.
        _race.add_runner(
            new ToddCoxeter(type,
                            S->rws()->isomorphic_non_fp_semigroup(),
                            genpairs,
                            ToddCoxeter::policy::use_cayley_graph));
        // Method 4: unlike with Method 2, this is not necessarily the same
        // as running Method 1, because the relations in
        // S->rws()->isomorphic_non_fp_semigroup() are likely not the same as
        // those in S->todd_coxeter()->isomorphic_non_fp_semigroup().
        // TODO:
        // - check if the relations are really the same as those in
        //   S->todd_coxeter(), if it exists. This is probably too expensive!
        // - we could just add the relations from the rws directly (rather than
        //   recreating them in the isomorphic_non_fp_semigroup, which is rather
        //   wasteful). If we do this, then this could be done outside the inner
        //   most if-statement here.
        _race.add_runner(
            new ToddCoxeter(type,
                            S->rws()->isomorphic_non_fp_semigroup(),
                            genpairs,
      ...
      // _race.add_runner(new P<RWSE*>(type, S->rws(), genpairs));
