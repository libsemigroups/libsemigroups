...
//                    0_w,     10_w,    1001_w,  1010_w,  1011_w, 1100_w,
//                    1101_w, 1110_w,  1_w,     0_w,     001_w,   010_w,  011_w,
//                    001_w, 0_w,     01_w,    00_w,    010_w,   011_w,  0_w,
//                    001_w, 011_w,   010_w,   00_w,    01_w,    100_w,  101_w,
//                    1011_w, 1010_w,  101_w,   100_w,   1001_w,  10_w,   110_w,
//                    11001_w, 11010_w, 11011_w, 11100_w, 11101_w, 10_w,
//                    11_w}));
//     }
//
//     LIBSEMIGROUPS_TEST_CASE("FpSemigroupInterface",
...
//       3})}); fp = std::make_unique<FpSemigroupByPairs<decltype(S)>>(S);
//       fp->add_rule({0, 0, 0}, {0});
//       fp->add_rule({1, 1, 1, 1}, {1});
//       fp->add_rule({0, 1, 0, 1}, {1, 1});
//       REQUIRE(!fp->finished());
...
//       using Transf = LeastTransf<5>;
//       FroidurePin<Transf> S({Transf({1, 3, 4, 2, 3}), Transf({3, 2, 1, 3,
//       3})}); SECTION("human readable alphabet") {
//         SECTION("ToddCoxeter") {
...
//       REQUIRE_NOTHROW(fp->add_rule(std::pair<word_type, word_type>({0},
//       {0}))); REQUIRE_NOTHROW(
...
//       }
//       fp->set_alphabet("ab");
//       REQUIRE_THROWS_AS(fp->add_rule("abaaaaaaaa", ""),
...
//       using Transf = LeastTransf<5>;
//       FroidurePin<Transf> S({Transf({1, 3, 4, 2, 3}), Transf({3, 2, 1, 3,
//       3})}); REQUIRE_THROWS_AS(fp->add_rules(S), LibsemigroupsException);
//     }
//
//     LIBSEMIGROUPS_TEST_CASE("FpSemigroupInterface",
//                             "012",
...
//       using Transf = LeastTransf<5>;
//       FroidurePin<Transf> S({Transf({1, 3, 4, 2, 3}), Transf({3, 2, 1, 3,
//       3})}); FpSemigroupByPairs<decltype(S)> fp(S);
...
//
//       using Transf = LeastTransf<5>;
//       FroidurePin<Transf> T({Transf({1, 3, 4, 2, 3}), Transf({3, 2, 1, 3,
//       3})}); REQUIRE_NOTHROW(fp.add_rules(T)); REQUIRE(fp.number_of_rules()
//       == 36); REQUIRE(fp.size() == S.size());
//       REQUIRE(fp.congruence().number_of_generating_pairs() == 0);
//     }
//
//     LIBSEMIGROUPS_TEST_CASE("FpSemigroupInterface",
