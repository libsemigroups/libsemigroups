...
namespace libsemigroups {
  ...
  LIBSEMIGROUPS_TEST_CASE("Ukkonen",
    ...
    v4::WordRange w;
    w.order(LenLexCmp(Alphabet<word_type>(6))).min(0).max(8);
    ...
    w.order(LenLexCmp(Alphabet<word_type>(5))).min(0).max(7);
    REQUIRE((w | rx::filter([&t](word_type const& u) {
  ...
  LIBSEMIGROUPS_TEST_CASE("Ukkonen",
    ...
    v4::WordRange words;
    words.order(LenLexCmp(Alphabet<word_type>(5))).min(0).max(5);
  ...
  LIBSEMIGROUPS_TEST_CASE("Ukkonen", "006", "dot", "[quick][ukkonen]") {
    ...
    }
    {
      ...
      v4::WordRange words;
      words.order(LenLexCmp(Alphabet<word_type>(2))).min(0).max(5);
