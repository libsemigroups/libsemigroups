...
namespace Catch {
  namespace Benchmark {
    namespace Detail {
      SampleAnalysis analyse(const IConfig& cfg,
                             FDuration*     first,
                             FDuration*     last) {
        if (!cfg.benchmarkNoAnalysis()) {
          ...
          return SampleAnalysis{
              ...
              Estimate<FDuration>{
                  FDuration(0), FDuration(0), FDuration(0), 0.0},
      ...
      namespace {
        ...
        static sample resample(URng&         rng,
          ...
          for (size_t i = 0; i < resamples; ++i) {
            ...
            const auto estimate = estimator(
                resampled.data(), resampled.data() + resampled.size());
...
namespace Catch {
  namespace Benchmark {
    namespace Detail {
      ...
      double
      weighted_average_quantile(int k, int q, double* first, double* last) {
      ...
      Estimate<double> bootstrap(double        confidence_level,
        ...
        auto a
            = [bias, accel](double b) { return bias + b / (1. - accel * b); };
        double b1 = bias + z1;
        double b2 = bias - z1;
        double a1 = a(b1);
        double a2 = a(b2);
      ...
      bootstrap_analysis analyse_samples(double       confidence_level,
        ...
        auto n = static_cast<int>(
            last - first);  // seriously, one can't use integral types without
                            // hell in C++
...
namespace Catch {
  ...
  namespace {
    ...
    static_assert(sizeof(TestCaseProperties) == sizeof(TCP_underlying_type),
                  "The size of the TestCaseProperties is different from the "
                  "assumed size");
  ...
  bool TestCaseInfo::okToFail() const {
    return applies(
        properties
        & (TestCaseProperties::ShouldFail | TestCaseProperties::MayFail));
  }
...
#include <cstddef>
  ...
  int StringMaker<double>::precision
  ...
  namespace Clara {
    namespace Detail {
      ...
      InternalParseResult ParserBase::parse(Args const& args) const {
        return parse(static_cast<std::string>(args.exeName()),
                     TokenStream(args));
      }
    ...
    ExeName::parse(std::string const&, Detail::TokenStream tokens) const {
      return Detail::InternalParseResult::ok(
          Detail::ParseState(ParseResultType::NoMatch, CATCH_MOVE(tokens)));
    }
...
namespace Catch {
  ...
  void
  ITransientExpression::streamReconstructedExpression(std::ostream& os) const {
    ...
    namespace {
      ...
      class DebugOutStream final : public IStream {
       ...
       public:
        DebugOutStream()
            : m_streamBuf(
                Detail::make_unique<StreamBufImpl<OutputDebugWriter>>()),
