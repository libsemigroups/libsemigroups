...
static double rdot_fma(const ae_int_t n,
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_fma(n, x->ptr.p_double, y->ptr.p_double, _state);
}
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_fma(n, x->ptr.p_double, a->ptr.pp_double[i], _state);
}
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_fma(n, a->ptr.pp_double[ia], b->ptr.pp_double[ib], _state);
}
...
static void raddv_fma_aligned(const ae_int_t n,
     const double alpha,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_fma_aligned(n, alpha, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_fma_aligned(n, alpha, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_fma_aligned(n, alpha, y->ptr.pp_double[ridx], x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_fma_aligned(n, alpha, y->ptr.pp_double[ridxsrc], x->ptr.pp_double[ridxdst]);
}
...
static void raddvx_fma_impl(const ae_int_t n, const double alpha, const double* __restrict y, double* __restrict x, ae_state *_state)
{
...
void raddvx_fma(ae_int_t n, double alpha, const ae_vector* y, ae_int_t offsy, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    raddvx_fma_impl(n, alpha, y->ptr.p_double+offsy, x->ptr.p_double+offsx, _state);
}

static void rmuladdv_fma_aligned(const ae_int_t n,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmuladdv_fma_aligned(n, y->ptr.p_double, z->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state* _state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        ...
        _mm256_storeu_pd(px+i, _mm256_fmadd_pd(yv, zv, xv));
    }
    for(; i<n; i++)
        px[i] += py[i]*pz[i];
}

static void rnegmuladdv_fma_aligned(const ae_int_t n,
     const double* __restrict y,
     const double* __restrict z,
     double* __restrict x)
{
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rnegmuladdv_fma_aligned(n, y->ptr.p_double, z->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state* _state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        ...
        const __m256d yv = _mm256_loadu_pd(py+i);
        const __m256d zv = _mm256_loadu_pd(pz+i);
        _mm256_storeu_pd(px+i, _mm256_fnmadd_pd(yv, zv, xv));
    }
    for(; i<n; i++)
        px[i] -= py[i]*pz[i];
}

static void rcopymuladdv_fma_aligned(const ae_int_t n,
     const double* __restrict y,
     const double* __restrict z,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopymuladdv_fma_aligned(n, y->ptr.p_double, z->ptr.p_double, x->ptr.p_double, r->ptr.p_double);
}
     ...
     ae_state* _state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        ...
        const __m256d yv = _mm256_loadu_pd(py+i);
        const __m256d zv = _mm256_loadu_pd(pz+i);
        _mm256_storeu_pd(pr+i, _mm256_fmadd_pd(yv, zv, xv));
    }
    for(; i<n; i++)
        pr[i] = px[i]+py[i]*pz[i];
}

static void rcopynegmuladdv_fma_aligned(const ae_int_t n,
     const double* __restrict y,
     const double* __restrict z,
     const double* __restrict x,
     double* __restrict r)
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopynegmuladdv_fma_aligned(n, y->ptr.p_double, z->ptr.p_double, x->ptr.p_double, r->ptr.p_double);
}
     ...
     ae_state* _state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        ...
        const __m256d yv = _mm256_loadu_pd(py+i);
        const __m256d zv = _mm256_loadu_pd(pz+i);
        _mm256_storeu_pd(pr+i, _mm256_fnmadd_pd(yv, zv, xv));
    }
    for(; i<n; i++)
        pr[i] = px[i]-py[i]*pz[i];
}
