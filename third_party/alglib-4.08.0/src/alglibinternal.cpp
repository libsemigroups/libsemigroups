...
*************************************************************************/
void nrpoolretrivechk(ae_nxpool* pool,
     /* Real    */ ae_vector* x,
     ae_int_t n,
     ae_state *_state)
{
    ...
    ae_nxpool_retrieve(pool, x, _state);
    ae_assert(x->cnt>=n, "nrPoolRetriveChk: retrieved array is too short", _state);
}
...
Retrieves an integer array from a pool and checks that it is long enough.
  ...
  -- ALGLIB --
     Copyright 06.05.2026 by Bochkanov Sergey
*************************************************************************/
void nipoolretrivechk(ae_nxpool* pool,
     /* Integer */ ae_vector* x,
     ae_int_t n,
     ae_state *_state)
{
    ...
    ae_nxpool_retrieve(pool, x, _state);
    ae_assert(x->cnt>=n, "niPoolRetriveChk: retrieved array is too short", _state);
}
...
Retrieves a boolean array from a pool and checks that it is long enough.
  ...
  -- ALGLIB --
     Copyright 06.05.2026 by Bochkanov Sergey
*************************************************************************/
void nbpoolretrivechk(ae_nxpool* pool,
     /* Boolean */ ae_vector* x,
     ae_int_t n,
     ae_state *_state)
{
    ...
    ae_nxpool_retrieve(pool, x, _state);
    ae_assert(x->cnt>=n, "nbPoolRetriveChk: retrieved array is too short", _state);
}
...
*************************************************************************/
void stimerrestart(stimer* t, ae_state *_state)
{
...
RESULT:
  ...
  -- ALGLIB --
     Copyright 20.01.2020 by Bochkanov Sergey
*************************************************************************/
void rscatteraddv(ae_int_t n,
     double alpha,
     /* Real    */ const ae_vector* y,
     /* Real    */ ae_vector* x,
     /* Integer */ const ae_vector* dstidx,
     ae_state *_state)
{
    ae_int_t i;
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_double[dstidx->ptr.p_int[i]] = x->ptr.p_double[dstidx->ptr.p_int[i]]+alpha*y->ptr.p_double[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_double[offsx+i] = x->ptr.p_double[offsx+i]+y->ptr.p_double[offsy+i]*z->ptr.p_double[offsz+i];
    }
}
#endif
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        r->ptr.p_double[offsr+i] = x->ptr.p_double[offsx+i]+y->ptr.p_double[offsy+i]*z->ptr.p_double[offsz+i];
    }
}
#endif
...
all three, provided that the corresponding offsets and spans are the same.
  ...
  -- ALGLIB --
     Copyright 29.10.2021 by Bochkanov Sergey
*************************************************************************/
void rcopynegmuladdvx(ae_int_t n,
     /* Real    */ const ae_vector* y,
     ae_int_t offsy,
     /* Real    */ const ae_vector* z,
     ae_int_t offsz,
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_double[offsx+i] = x->ptr.p_double[offsx+i]*y->ptr.p_double[offsy+i];
    }
}
#endif
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_double[i] = ae_maxreal(x->ptr.p_double[i], ae_fabs(y->ptr.p_double[i], _state), _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[i] = ae_maxreal((double)(1), v*ae_fabs(x->ptr.p_double[i], _state), _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[i] = f/ae_sqr(ae_maxreal((double)(1), v*ae_fabs(x->ptr.p_double[i], _state), _state), _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[i] = x->ptr.p_double[dstidx->ptr.p_int[i]];
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[dstidx->ptr.p_int[i]] = x->ptr.p_double[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[i+offsy] = x->ptr.p_double[dstidx->ptr.p_int[i]+offsx];
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[i] = 1.0/x->ptr.p_double[i];
    }
}
#endif
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        y->ptr.p_double[offsy+i] = 1.0/x->ptr.p_double[offsx+i];
    }
}
#endif
    ...
    if( isforward )
    {
        if( n1!=n2 )
        {
            ...
            for(j=m1; j<=m2-1; j++)
            {
                ...
                if( ctemp!=(double)1||stemp!=(double)0 )
                {
                    jp1 = j+1;
    ...
    *r = 0.0;
    ...
    else
    {
        ...
        else
        {
            f1 = f;
            g1 = g;
            af1 = ae_fabs(f1, _state);
            ag1 = ae_fabs(g1, _state);
            if( af1>ag1 )
            {
                v = g1/f1;
                *r = af1*ae_sqrt((double)1+v*v, _state);
            }
            else
            {
                v = f1/g1;
                *r = ag1*ae_sqrt((double)1+v*v, _state);
            }
            *cs = f1/(*r);
            *sn = g1/(*r);
            if( af1>ag1&&*cs<(double)0 )
            {
                *cs = -*cs;
                *sn = -*sn;
                *r = -*r;
...
ae_bool armijoiteration(armijostate* state, ae_state *_state)
{
    ...
    if( state->rstate.rcomm2_handler!=NULL && state->rstate.requesttype!=0 && state->rstate.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        state->rstate.rcomm2_handler(&state->rstate, state->rstate.handler_p0, state->rstate.handler_p1, state->rstate.handler_p2, state->rstate.handler_p3, _state);
    else
        goto lbl_rcomm;
lbl_0:
