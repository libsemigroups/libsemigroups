...
static double rdot_sse2(ae_int_t n,
     const double* px,
     const double* py,
     ae_state *_state)
{
    ...
    const __m128d* __restrict pX = (const __m128d*)(px);
    const __m128d* __restrict pY = (const __m128d*)(py);
    __m128d ans;
    if(unrollLen >= 8) {
     ...
     ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_sse2(n, x->ptr.p_double, y->ptr.p_double, _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_sse2(n, x->ptr.p_double, a->ptr.pp_double[i], _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_sse2(n, a->ptr.pp_double[ia], b->ptr.pp_double[ib], _state);
}
     ...
     ae_state *_state)
{
    ...
    const ae_int_t tail = sse2len<<1;
    if(n-tail) {
        return scalar + px[tail]*px[tail];
    } else {
        return scalar;
    }
}

static void rcopyv_sse2_aligned(const ae_int_t n,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_sse2_aligned(n, x->ptr.p_double, y->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_sse2_aligned(n, x->ptr.p_double, a->ptr.pp_double[i]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_sse2_aligned(n, a->ptr.pp_double[i], x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_sse2_aligned(n, a->ptr.pp_double[i], b->ptr.pp_double[k]);
}

static void rcopymulv_sse2_aligned(const ae_int_t n,
     const double v,
     /* Real    */ const double* __restrict x,
     /* Real    */ double* __restrict y)
{
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopymulv_sse2_aligned(n, v, x->ptr.p_double, y->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopymulv_sse2_aligned(n, v, x->ptr.p_double, y->ptr.pp_double[ridx]);
}

static void icopyv_sse2_aligned(const ae_int_t n, const ae_int_t* __restrict x,
...
void icopyv_sse2(ae_int_t n, const ae_vector* x, ae_vector* y, ae_state *_state)
{
    if( n==0 )
        return;
    icopyv_sse2_aligned(n, x->ptr.p_int, y->ptr.p_int);
}

static void bcopyv_sse2_aligned(const ae_int_t n, const ae_bool* __restrict x,
...
void bcopyv_sse2(ae_int_t n, const ae_vector* x, ae_vector* y, ae_state *_state)
{
    if( n==0 )
        return;
    bcopyv_sse2_aligned(n, x->ptr.p_bool, y->ptr.p_bool);
}

static void rsetv_sse2_aligned(const ae_int_t n,
     const double v,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rsetv_sse2_aligned(n, v, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rsetv_sse2_aligned(n, v, a->ptr.pp_double[i]);
}

static void rsetvx_sse2_impl(const ae_int_t n,
     const double v,
     /* Real    */ double* __restrict x,
     const ae_state* __restrict _state)
{
    ...
    if((((ptrdiff_t)x) & 15) == 0)
    {
        rsetv_sse2_aligned(n, v, x);
        return;
    }
    x[0] = v;
    rsetv_sse2_aligned(n-1, v, x+1);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rsetvx_sse2_impl(n, v, x->ptr.p_double+offsx, _state);
}

static void isetv_sse2_aligned(const ae_int_t n, const ae_int_t v,
...
void isetv_sse2(ae_int_t n, ae_int_t v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    isetv_sse2_aligned(n, v, x->ptr.p_int);
}

static void bsetv_sse2_aligned(const ae_int_t n, const ae_bool v, ae_bool* __restrict x)
{
    const ae_int_t tail = n & 15;
    const ae_int_t even = n-tail;
    __m128i *__restrict pDest = (__m128i*)x;
...
void bsetv_sse2(ae_int_t n, ae_bool v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    bsetv_sse2_aligned(n, v, x->ptr.p_bool);
}

static void rmulv_sse2_aligned(const ae_int_t n, const double v, double* __restrict x)
{
    ae_int_t i;
...
void rmulv_sse2(ae_int_t n, double v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    rmulv_sse2_aligned(n, v, x->ptr.p_double);
}
...
void rmulr_sse2(ae_int_t n, double v, ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return;
    rmulv_sse2_aligned(n, v, x->ptr.pp_double[rowidx]);
}

static void rmulvx_sse2_impl(const ae_int_t n, const double v, double* __restrict x,
    const ae_state* __restrict _state)
{
    if( n<=4 )
    {
    ...
    if((((ptrdiff_t)x) & 15) == 0)
    {
        rmulv_sse2_aligned(n, v, x);
        return;
    }
    x[0] = v * x[0];
    rmulv_sse2_aligned(n-1, v, x+1);
}
...
void rmulvx_sse2(ae_int_t n, double v, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    rmulvx_sse2_impl(n, v, x->ptr.p_double+offsx, _state);
}

static void raddv_sse2_aligned(const ae_int_t n,
     const double alpha,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_sse2_aligned(n, alpha, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_sse2_aligned(n, alpha, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_sse2_aligned(n, alpha, y->ptr.pp_double[ridx], x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_sse2_aligned(n, alpha, y->ptr.pp_double[ridxsrc], x->ptr.pp_double[ridxdst]);
}
...
static void raddvx_sse2_impl(const ae_int_t n, const double alpha,
    const double* __restrict y, double* __restrict x, ae_state *_state)
{
    if( n<=4 )
    {
    ...
    const ae_vector* y, ae_int_t offsy, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    raddvx_sse2_impl(n, alpha, y->ptr.p_double+offsy, x->ptr.p_double+offsx, _state);
}

static void rmergemulv_sse2_aligned(const ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x)
{
    ae_int_t i;
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_sse2_aligned(n, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_sse2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_sse2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double);
}

static void rmergemaxv_sse2_aligned(const ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_sse2_aligned(n, y->ptr.p_double, x->ptr.p_double, _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_sse2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx], _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_sse2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double, _state);
}

static void rmergeminv_sse2_aligned(const ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x,
     ae_state* __restrict _state)
{
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_sse2_aligned(n, y->ptr.p_double, x->ptr.p_double, _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_sse2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx], _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_sse2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double, _state);
}

static double rmaxv_sse2_aligned(ae_int_t n, /* Real    */ const double* __restrict x, ae_state* __restrict _state)
{
    ae_int_t i;
...
double rmaxv_sse2(ae_int_t n, /* Real    */ const ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxv_sse2_aligned(n, x->ptr.p_double, _state);
}
...
double rmaxr_sse2(ae_int_t n, /* Real    */ const ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxv_sse2_aligned(n, x->ptr.pp_double[rowidx], _state);
}

static double rmaxabsv_sse2_aligned(ae_int_t n, /* Real    */ const double* __restrict x, ae_state* __restrict _state)
{
...
double rmaxabsv_sse2(ae_int_t n, /* Real    */ const ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxabsv_sse2_aligned(n, x->ptr.p_double, _state);
}
...
double rmaxabsr_sse2(ae_int_t n, /* Real    */ const ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxabsv_sse2_aligned(n, x->ptr.pp_double[rowidx], _state);
}
...
static void rcopyvx_sse2_impl(const ae_int_t n, const double* __restrict x,
    double* __restrict y, ae_state *_state)
{
    if((((ptrdiff_t)x) & 15) == 0)
    {
    ...
    ae_vector* y, ae_int_t offsy, ae_state *_state)
{
    if( n==0 )
        return;
    rcopyvx_sse2_impl(n, x->ptr.p_double+offsx, y->ptr.p_double+offsy, _state);
}
...
static void icopyvx_sse2_impl(const ae_int_t n, const ae_int_t* __restrict x,
                ae_int_t* __restrict y, ae_state* __restrict _state)
{
                ...
                ae_vector* y, ae_int_t offsy, ae_state *_state)
{
    if( n==0 )
        return;
    icopyvx_sse2_impl(n, x->ptr.p_int+offsx, y->ptr.p_int+offsy, _state);
}
