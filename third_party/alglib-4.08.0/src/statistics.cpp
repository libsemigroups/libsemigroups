    ...
    const xparams _xparams)
{
    ...
    alglib_impl::rcommv2_request request(&optimizer.rcommv2, ptr, "mcmc");
    alglib_impl::rcommv2_callbacks callbacks;
    alglib_impl::rcommv2_buffers buffers(&state.c_ptr()->rcommv2.tmpx1, &state.c_ptr()->rcommv2.tmpc1, &state.c_ptr()->rcommv2.tmpf1, &state.c_ptr()->rcommv2.tmpg1, &state.c_ptr()->rcommv2.tmpj1, &state.c_ptr()->rcommv2.tmps1);
    jmp_buf _break_jump;
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    if( setjmp(_break_jump) )
...
callbacks.func = func;
    ...
    alglib_impl::_rcommstate_assign_handler(&state.c_ptr()->rcommv2, alglib_impl::rcommv2_request_cpphandler, &request, &callbacks, &buffers, NULL);
    while( alglib_impl::mcmciteration(state.c_ptr(), &_alglib_env_state) )
    {
        _ALGLIB_CALLBACK_EXCEPTION_GUARD_BEGIN
        alglib_impl::ae_assert(ae_false, "ALGLIB: critical error in 'mcmcrun' (RCommV2 request escaped handler)", &_alglib_env_state);
...
NOTE: this function also tells the sampler not to  re-seed  internal  RNG,
  ...
  -- ALGLIB --
     Copyright 15.05.2025 by Bochkanov Sergey
*************************************************************************/
bool mcmcrestart(mcmcstate &state, const ae_int_t epochscnt, const xparams _xparams)
{
    jmp_buf _break_jump;
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    ...
    if( _xparams.flags!=(alglib_impl::ae_uint64_t)0x0 )
        ae_state_set_flags(&_alglib_env_state, _xparams.flags);
    ae_bool result = alglib_impl::mcmcrestart(state.c_ptr(), epochscnt, &_alglib_env_state);
    alglib_impl::ae_state_clear(&_alglib_env_state);
    return bool(result);
}
...
ae_bool mcmciteration(mcmcstate* state, ae_state *_state)
{
    ...
    if( state->rcommv2.stage>=0 )
    {
        ...
        j = state->rcommv2.ia.ptr.p_int[2];
        k = state->rcommv2.ia.ptr.p_int[3];
        itidx = state->rcommv2.ia.ptr.p_int[4];
        itmax = state->rcommv2.ia.ptr.p_int[5];
        groupscnt = state->rcommv2.ia.ptr.p_int[6];
        grpdstsize = state->rcommv2.ia.ptr.p_int[7];
        dst = state->rcommv2.ia.ptr.p_int[8];
        accept1cnt = state->rcommv2.ia.ptr.p_int[9];
        accepthcnt = state->rcommv2.ia.ptr.p_int[10];
        v = state->rcommv2.ra.ptr.p_double[0];
    }
    else
    {
        n = 359;
    ...
    rallocv(n, &state->rcommv2.querydata, _state);
    rallocv(1, &state->rcommv2.replyfi, _state);
    rallocv(1, &state->rcommv2.tmpf1, _state);
    rallocv(n, &state->rcommv2.tmpx1, _state);
    rallocv(n, &state->rcommv2.tmpg1, _state);
    ae_vector_set_length(&state->rcommv2.reportx, 1, _state);
    state->rcommv2.reportx.ptr.p_double[0] = (double)(0);
    state->rcommv2.reportf = (double)(0);
...
lbl_4:
    ...
    for(j=0; j<=n-1; j++)
    {
        state->rcommv2.querydata.ptr.p_double[j] = state->population2d.ptr.pp_double[i][j];
    }
    stimerstartcond(&state->timercallback, state->dotimers, _state);
    state->rcommv2.stage = 0;
    if( state->rcommv2.rcomm2_handler!=NULL && state->rcommv2.requesttype!=0 && state->rcommv2.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        state->rcommv2.rcomm2_handler(&state->rcommv2, state->rcommv2.handler_p0, state->rcommv2.handler_p1, state->rcommv2.handler_p2, state->rcommv2.handler_p3, _state);
    else
        goto lbl_rcomm;
lbl_0:
    stimerstopcond(&state->timercallback, state->dotimers, _state);
    state->population2d.ptr.pp_double[i][n] = state->rcommv2.replyfi.ptr.p_double[0];
    i = i+1;
    goto lbl_4;
lbl_6:
    if( !state->xrep )
...
lbl_15:
    ...
    rcopyrv(n, &state->propxf, j, &state->rcommv2.querydata, _state);
    state->repnfev = state->repnfev+1;
    state->rcommv2.requesttype = 4;
    state->rcommv2.queryfuncs = 1;
    state->rcommv2.queryvars = n;
...
lbl_2:
    stimerstopcond(&state->timercallback, state->dotimers, _state);
    state->propxf.ptr.pp_double[j][n] = state->rcommv2.replyfi.ptr.p_double[0];
    j = j+1;
    goto lbl_15;
lbl_17:
...
lbl_rcomm:
    ...
    state->rcommv2.ia.ptr.p_int[2] = j;
    state->rcommv2.ia.ptr.p_int[3] = k;
    state->rcommv2.ia.ptr.p_int[4] = itidx;
    state->rcommv2.ia.ptr.p_int[5] = itmax;
    state->rcommv2.ia.ptr.p_int[6] = groupscnt;
    state->rcommv2.ia.ptr.p_int[7] = grpdstsize;
    state->rcommv2.ia.ptr.p_int[8] = dst;
    state->rcommv2.ia.ptr.p_int[9] = accept1cnt;
    state->rcommv2.ia.ptr.p_int[10] = accepthcnt;
    state->rcommv2.ra.ptr.p_double[0] = v;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(epochscnt>=0, "MCMCRestart: EpochsCnt<0", _state);
    ...
    state->initialstart = ae_false;
    if( epochscnt>0 )
    {
        state->epochscnt = epochscnt;
    }
    ae_vector_set_length(&state->rcommv2.ia, 10+1, _state);
    ae_vector_set_length(&state->rcommv2.ra, 0+1, _state);
    state->rcommv2.stage = -1;
