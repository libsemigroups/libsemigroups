...
static double rdot_avx2(const ae_int_t n,
    ...
    ae_state *_state)
{
    ...
    const __m256d* __restrict pX = (const __m256d*)(px);
    const __m256d* __restrict pY = (const __m256d*)(py);
    __m256d ans;
    if(fmaLen >= 4) {
        __m256d fmaUnroll[4];
    ...
    case 2: {
        dot += px[tail+0]*py[tail+0];
        dot += px[tail+1]*py[tail+1];
        break;
    }
    case 3: {
        dot += px[tail+0]*py[tail+0];
        dot += px[tail+1]*py[tail+1];
        dot += px[tail+2]*py[tail+2];
        break;
    }
    }
    return dot;
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_avx2(n, x->ptr.p_double, y->ptr.p_double, _state);
}
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_avx2(n, x->ptr.p_double, a->ptr.pp_double[i], _state);
}
    ...
    ae_state *_state)
{
    if( n==0 )
        return 0;
    return rdot_avx2(n, a->ptr.pp_double[ia], b->ptr.pp_double[ib], _state);
}
    ...
    ae_state *_state)
{
    ...
    case 2: {
        dot += px[tail+0]*px[tail+0];
        dot += px[tail+1]*px[tail+1];
        break;
    }
    case 3: {
        dot += px[tail+0]*px[tail+0];
        dot += px[tail+1]*px[tail+1];
        dot += px[tail+2]*px[tail+2];
        break;
    }
    }
    return dot;
...
static void rcopyv_avx2_aligned(ae_int_t n,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_avx2_aligned(n, x->ptr.p_double, y->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_avx2_aligned(n, x->ptr.p_double, a->ptr.pp_double[i]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_avx2_aligned(n, a->ptr.pp_double[i], x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopyv_avx2_aligned(n, a->ptr.pp_double[i], b->ptr.pp_double[k]);
}

static void rcopymulv_avx2_aligned(const ae_int_t n,
     const double v,
     /* Real    */ const double* __restrict x,
     /* Real    */ double* __restrict y)
{
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopymulv_avx2_aligned(n, v, x->ptr.p_double, y->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rcopymulv_avx2_aligned(n, v, x->ptr.p_double, y->ptr.pp_double[ridx]);
}
     ...
     ae_state *_state)
{
    ...
    const ae_int_t avx2len = n>>2;
    const ae_int_t tail = avx2len<<2;
    const __m256d avx2one = _mm256_set1_pd(1.0);
    
    for(i=0; i<avx2len; i++)
    {
        const ae_int_t j = i<<2;
        _mm256_storeu_pd(py+j, _mm256_div_pd(avx2one, _mm256_loadu_pd(px+j)));
    }
    switch(n-tail)
    {
    case 1:
        ...
        break;
    case 2: {
        const __m128d one = _mm256_extractf128_pd(avx2one, 0);
        _mm_storeu_pd(py+tail, _mm_div_pd(one, _mm_loadu_pd(px+tail)));
        break;
    }
    case 3: {
        const __m128d one = _mm256_extractf128_pd(avx2one, 0);
        _mm_storeu_pd(py+tail, _mm_div_pd(one, _mm_loadu_pd(px+tail)));
        py[tail+2] = 1.0/px[tail+2];
        break;
    }
    }
}
...
static void icopyv_avx2_aligned(const ae_int_t n, const ae_int_t* __restrict x,
...
void icopyv_avx2(ae_int_t n, const ae_vector* x, ae_vector* y, ae_state *_state)
{
    if( n==0 )
        return;
    icopyv_avx2_aligned(n, x->ptr.p_int, y->ptr.p_int);
}

static void bcopyv_avx2_aligned(const ae_int_t n, const ae_bool* __restrict x,
...
void bcopyv_avx2(ae_int_t n, const ae_vector* x, ae_vector* y, ae_state *_state)
{
    if( n==0 )
        return;
    bcopyv_avx2_aligned(n, x->ptr.p_bool, y->ptr.p_bool);
}

static void rsetv_avx2_aligned(const ae_int_t n,
     const double v,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rsetv_avx2_aligned(n, v, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rsetv_avx2_aligned(n, v, a->ptr.pp_double[i]);
}

static void rsetvx_avx2_impl(const ae_int_t n, const double v, double* __restrict x,
    const ae_state* __restrict _state)
{
    ...
    switch(unal)
    {
        ...
        case 24:
        {
            ...
            rsetv_avx2_aligned(n-nDone, v, x+nDone);
            return;
        }
    }
}
...
void rsetvx_avx2(ae_int_t n, double v, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    rsetvx_avx2_impl(n, v, x->ptr.p_double+offsx, _state);
}

static void isetv_avx2_aligned(const ae_int_t n, const ae_int_t v,
...
void isetv_avx2(ae_int_t n, ae_int_t v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    isetv_avx2_aligned(n, v, x->ptr.p_int);
}

static void bsetv_avx2_aligned(const ae_int_t n, const ae_bool v, ae_bool* __restrict x)
{
    const ae_int_t tail = n & 31;
    const ae_int_t even = n-tail;
    __m256i *__restrict pDest = (__m256i*)x;
...
void bsetv_avx2(ae_int_t n, ae_bool v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    bsetv_avx2_aligned(n, v, x->ptr.p_bool);
}

static void rmulv_avx2_aligned(const ae_int_t n, const double v, double* __restrict x)
{
    ae_int_t i;
...
void rmulv_avx2(ae_int_t n, double v, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    rmulv_avx2_aligned(n, v, x->ptr.p_double);
}
...
void rmulr_avx2(ae_int_t n, double v, ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return;
    rmulv_avx2_aligned(n, v, x->ptr.pp_double[rowidx]);
}

static void rsqrtv_avx2_aligned(const ae_int_t n, double* __restrict x)
{
    ae_int_t i;
...
void rsqrtv_avx2(ae_int_t n, ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return;
    rsqrtv_avx2_aligned(n, x->ptr.p_double);
}
...
void rsqrtr_avx2(ae_int_t n, ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return;
    rsqrtv_avx2_aligned(n, x->ptr.pp_double[rowidx]);
}

static void rmulvx_avx2_impl(const ae_int_t n, const double v, double* __restrict x,
    const ae_state* __restrict _state)
{
    const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
    if( n<=4 )
    ...
    case 24: {
        x[0] = v*x[0];
        const ptrdiff_t nDone = 4-(unal>>3);
        rmulv_avx2_aligned(n-nDone, v, x+nDone);
        return;
    }
    }
}
...
void rmulvx_avx2(ae_int_t n, double v, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    rmulvx_avx2_impl(n, v, x->ptr.p_double+offsx, _state);
}

static void raddv_avx2_aligned(const ae_int_t n,
     const double alpha,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_avx2_aligned(n, alpha, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_avx2_aligned(n, alpha, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_avx2_aligned(n, alpha, y->ptr.pp_double[ridx], x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    raddv_avx2_aligned(n, alpha, y->ptr.pp_double[ridxsrc], x->ptr.pp_double[ridxdst]);
}
...
static void raddvx_avx2_impl(const ae_int_t n, const double alpha, const double* __restrict y,
    ...
    ae_int_t offsy, ae_vector* x, ae_int_t offsx, ae_state *_state)
{
    if( n==0 )
        return;
    raddvx_avx2_impl(n, alpha, y->ptr.p_double+offsy, x->ptr.p_double+offsx, _state);
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        const __m256d xv = _mm256_loadu_pd(px+i);
        const __m256d yv = _mm256_loadu_pd(py+i);
        const __m256d zv = _mm256_loadu_pd(pz+i);
        _mm256_storeu_pd(px+i, _mm256_add_pd(xv, _mm256_mul_pd(yv, zv)));
    }
    for(; i<n; i++)
        px[i] += py[i]*pz[i];
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        ...
        const __m256d yv = _mm256_loadu_pd(py+i);
        const __m256d zv = _mm256_loadu_pd(pz+i);
        _mm256_storeu_pd(pr+i, _mm256_sub_pd(xv, _mm256_mul_pd(yv, zv)));
    }
    for(; i<n; i++)
        pr[i] = px[i]-py[i]*pz[i];
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<vecLen; i+=4)
    {
        const __m256d xv = _mm256_loadu_pd(px+i);
        const __m256d yv = _mm256_loadu_pd(py+i);
        _mm256_storeu_pd(px+i, _mm256_mul_pd(xv, yv));
    }
    for(; i<n; i++)
        px[i] *= py[i];
}

static void rmergemulv_avx2_aligned(ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x)
{
    ae_int_t i;
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_avx2_aligned(n, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_avx2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemulv_avx2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double);
}

static void rmergedivv_avx2_aligned(ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x)
{
    ae_int_t i;
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergedivv_avx2_aligned(n, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergedivv_avx2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergedivv_avx2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double);
}

static void rmergemaxv_avx2_aligned(ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x)
{
    ae_int_t i;
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_avx2_aligned(n, y->ptr.p_double, x->ptr.p_double);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_avx2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx]);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergemaxv_avx2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double);
}

static void rmergeminv_avx2_aligned(ae_int_t n,
     /* Real    */ const double* __restrict y,
     /* Real    */ double* __restrict x,
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_avx2_aligned(n, y->ptr.p_double, x->ptr.p_double, _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_avx2_aligned(n, y->ptr.p_double, x->ptr.pp_double[rowidx], _state);
}
     ...
     ae_state *_state)
{
    if( n==0 )
        return;
    rmergeminv_avx2_aligned(n, y->ptr.pp_double[rowidx], x->ptr.p_double, _state);
}

static double rmaxv_avx2_aligned(ae_int_t n, /* Real    */ const double* __restrict x, ae_state* __restrict _state)
{
    ae_int_t i;
    const ae_int_t avx2len = n>>2;
    const ae_int_t tail = avx2len<<2;
...
double rmaxv_avx2(ae_int_t n, /* Real    */ const ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxv_avx2_aligned(n, x->ptr.p_double, _state);
}
...
double rmaxr_avx2(ae_int_t n, /* Real    */ const ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxv_avx2_aligned(n, x->ptr.pp_double[rowidx], _state);
}

static double rmaxabsv_avx2_aligned(ae_int_t n, /* Real    */ const double* __restrict x, ae_state* __restrict _state)
{
...
double rmaxabsv_avx2(ae_int_t n, /* Real    */ const ae_vector* x, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxabsv_avx2_aligned(n, x->ptr.p_double, _state);
}
...
double rmaxabsr_avx2(ae_int_t n, /* Real    */ const ae_matrix* x, ae_int_t rowidx, ae_state *_state)
{
    if( n==0 )
        return 0.0;
    return rmaxabsv_avx2_aligned(n, x->ptr.pp_double[rowidx], _state);
}
...
static void rcopyvx_avx2_impl(const ae_int_t n, const double* __restrict x,
    double* __restrict y, ae_state *_state)
{
    const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
    if( n<=4 )
    ...
    ae_vector* y, ae_int_t offsy, ae_state *_state)
{
    if( n==0 )
        return;
    rcopyvx_avx2_impl(n, x->ptr.p_double+offsx, y->ptr.p_double+offsy, _state);
}
...
static void icopyvx_avx2_impl(const ae_int_t n, const ae_int_t* __restrict x,
                ae_int_t* __restrict y, ae_state* __restrict _state)
{
    const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
    if( n<=8 )
                ...
                ae_vector* y, ae_int_t offsy, ae_state *_state)
{
    if( n==0 )
        return;
    icopyvx_avx2_impl(n, x->ptr.p_int+offsx, y->ptr.p_int+offsy, _state);
}
