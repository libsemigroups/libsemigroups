...
// This override is provided to avoid name clashes in foreign codebases where
// `rx` already has a different meaning.
...
// Some foreign libraries may have their own optional type, which may be
// optimized for their own value types. For example, some libraries treat NaN
// values as "none", or provide "transparent" optional when the value type has
// an "illegal" representation.
...
// Some foreign libraries may provide a more user-friendly static assertion
// facility. Most compilers don't give any context about the types that failed a
// `static_assert()` check in templated code.
...
// Some foreign libraries may provide their own assertion facilities that
// integrate better with their environment.
...
// Unfortunately, this is not compatible with C++20 `[[likely]]` /
// `[[unlikely]]`, because it has a different syntax structure.
...
/*!
    ...
    - Input range: An object that can produce a series of values. Input ranges
   can be chained together and combined to form aggregate input ranges.

    - Sink:        An object that can receive values from input ranges in order
   to perform some action on the whole range of results. A chain of sinks will
   operate on the same resulting container without allocating any intermediate
   storage. Sinks can also be (implicitly) converted to input ranges, in which
   case temporary storage *will* be allocated for the intermediate result.
...
namespace RX_NAMESPACE {
  ...
  /*!
      ...
      @code
      // Required:
      using output_type = ...; // The output element type, usually typename
     Inner::output_type.

      output_type get() const noexcept; // Get the value at the current
     position.
      ...
      size_t size_hint() const noexcept; // A hint about how many times `next()`
     can be called.
                                         // It is purely an optimization hint,
     not the accurate number
                                         // of times. Return
     `std::numeric_limits<size_t>::max()` if
                                         // unknown.

      // Optional:

      static constexpr bool is_finite = ...; // True if the range produces a
     bounded number of
                                             // elements. Defaults to false.
      static constexpr bool is_idempotent = ...; // True if `get()` can be
     called multiple times
                                                 // without advancing internal
     state. Defaults to
                                                 // false.
      T&& get() && noexcept; // For non-idempotent ranges, an rvalue reference
     may be returned for
                             // efficiency. Note that this also requires the
     const `get()` version to
                             // be marked as `const&` instead of just `const`.
      size_t advance_by(size_t) noexcept; // Advance by n positions, or until
     the range is at its end.
      @endcode

      Calling `get()` or `next()` while `at_end() == true` is a breach of
     contract, and is allowed to be undefined behavior.
  ...
  */
  ...
  template <class T>
  struct is_input_range<T,
                        std::void_t<typename T::output_type,
  ...
  /*!
      ...
      Sinks transfer the result of a range expression to a container. If a sink
     is used as an InputRange, the result will be copied to a temporary vector,
     which will then become the input.

      A sink is expected to conform to the following interface:

      @code
      using output_type = ...; // The output element type, usually `typename
     Inner::output_type`, if
                               // the sink is wrapping another range or sink.
      template <class Out>
      void sink(Out& out) && noexcept; // Call sink(std::move(inner), out), and
     perform the desired
                                       // operation on the result.
      @endcode

      The `sink()` member function should be "destructive", and does not need to
     be repeatable multiple times for the same instance. If the sink wraps
     another range, it should call `sink(std::move(inner), out)`.
  ...
  */
  ...
  // Test whether T is a sink by checking that it has `T::output_type` and an
  // implementation of `sink()` that accepts an `std::vector<T::output_type>&`
  // as its argument.
  template <class T>
  struct is_sink<T,
                 std::void_t<decltype(std::declval<T&&>().sink(
                     std::declval<std::vector<
                         typename remove_cvref_t<T>::output_type>&>()))>>
      : std::true_type {};
  template <class T>
  ...
  constexpr bool has_emplace_back_v
      = has_emplace_back<remove_cvref_t<T>>::value;
  ...
  // Trait for input ranges to indicate whether they output a finite number of
  // elements.
  template <class T, class Enable = void>
  struct is_finite : std::false_type {};
  template <class T>
  struct is_finite<T, std::void_t<decltype(T::is_finite)>>
      : std::bool_constant<T::is_finite> {};
  template <class T>
  constexpr bool is_finite_v = is_finite<remove_cvref_t<T>>::value;

  // Trait for input ranges to indicate whether their `get()` function can be
  // called multiple times without advancing internal state.
  template <class T, class Enable = void>
  ...
  // Fake RandomAccessIterator concept. Any iterator that supports advancing by
  // an arbitrary integer amount is considered "random access".
  template <class T, class Enable = void>
  struct is_random_access_iterator : std::false_type {};
  template <class T>
  struct is_random_access_iterator<
      T,
      std::void_t<decltype(std::declval<T&>() += 0)>> : std::true_type {};
  template <class T>
  ...
  ///
  /// Note that this enables `operator|` for any pair where RHS is callable with
  /// LHS as its only argument. Therefore, it may be best to selectively import
  /// this operator into your scope with `using`. If it simply cannot be
  /// imported without clashing with other definitions of `operator|`, you can
  /// use the alternative chaining syntax using `operator()`: `rhs(lhs)`.
  template <
      class LHS,
      class RHS,
  ...
  */
  ...
  struct input_range_iterator {
    ...
    template <class T = typename R::output_type,
  ...
  template <class T>
  struct has_advance_by<
      T,
      std::void_t<decltype(std::declval<T&>().advance_by(size_t(0)))>>
      : std::true_type {};
  template <class T>
  ...
  ///
  /// This calls R::advance_by() if available. Otherwise, it calls R::next() at
  /// most n times.
  template <class R>
  ...
  constexpr void sink_one(In&& in, Out& out) noexcept {
    ...
    // Copy elements from the input to the output. If the input is tuple-like,
    // and the output supports emplacement, the tuple will be unpacked and
    // passed as individual arguments to the emplace member function on the
    // output. Otherwise, the tuple-like object will be passed as-is.
    //
    // This means that both linear containers of tuples and associative
    // containers like maps will work as outputs.

    if constexpr (has_emplace_back_v<Out>) {
      if constexpr (is_tuple_like_v<In>) {
        // Output has emplace_back, and the input generates tuple-like elements.
        // Pass tuple elements as individual arguments to emplace_back.
      ...
      if constexpr (is_tuple_like_v<In>) {
        // Output has emplace, and the input generates tuple-like elements. Pass
        // tuple elements as individual arguments to emplace.
        auto unpack = [&](auto&&... args) constexpr {
  ...
  ///
  /// If an input range is chained with a sink (like `sort()`), this will the
  /// initial population of the output, on which any further "sink" operators
  /// (like `uniq()`) will operate.
  ///
  /// This function only participates in overload resolution if @a in is an
  /// rvalue reference. This is to provide a compile-time check for reentrancy
  /// -- `sink()` should never advance the state of an input range that may
  /// later be reused by the caller.
  ///
  /// Note: As of C++17, no standard container can actually allocate memory in a
  /// `constexpr` context,
  ///       but this was added in C++20. This is the only reason that this
  ///       function is marked `constexpr`. See:
  ///       http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2019/p0784r5.html
  ///
  ...
  /// Copy elements from an lvalue InputRange to sink. Note that the input range
  /// is copied to maintain reentrancy.
  ...
  /// Copy resulting elements from an rvalue sink implementation into the output
  /// container.
  ///
  /// This function is what allows operations that require temporary storage to
  /// work on the resulting container without allocating their own temporary
  /// storage.
  ///
  /// @tparam In The sink to copy from, typically an output modifier like `sort`
  /// or `uniq`.
  /// @tparam Out A container that can hold the result.
  template <class In,
            class Out,
  ...
  /// Copy resulting elements from an lvalue range, copying the input range
  /// before modifying it.
  ///
  /// Note: This function should not be called from a Sink's implementation of
  /// `sink(out)`. Instead, call `sink(std::move(inner), out)` to avoid
  /// unnecessary copies.
  template <class In, class Out>
  constexpr void sink(const In& in,
                      Out&      out,
                      std::enable_if_t<is_sink_v<In>>* = nullptr) noexcept {
    In copy = in;
    sink(std::move(copy), out);
  }
  ...
  sink(const In& in,
       Out&      out,
       std::enable_if_t<!is_input_or_sink_v<In>>* = nullptr) noexcept {
    std::copy(begin(in), end(in), std::back_inserter(out));
  }

  /*!
      @brief Create a range from a standard iterator pair, or a standard
     container.
  ...
  */
  ...
  struct iterator_range {
    ...
    // TODO: If the end-iterator type is different from the iterator type, this
    // may not produce a finite range.
    static constexpr bool is_finite = true;
    static constexpr bool is_idempotent
        = true;  // we only operate on const iterators
    ...
    constexpr iterator_range(It begin, EndIt end) noexcept
  ...
  template <class C>
  iterator_range(const C& c)
      -> iterator_range<decltype(begin(c)), decltype(end(c))>;
  template <class It, class EndIt>
  iterator_range(It&&, EndIt&&)
      -> iterator_range<remove_cvref_t<It>, remove_cvref_t<EndIt>>;

  /*!
      @brief An input range over an internally owned `std::vector`.

      This is used internally when chains require temporary storage (for
     example, when a sink is used as an input rage).

      @tparam T The element type of the internal vector. Must be a non-reference
     value type.
  */
  template <class T>
  struct vector_range {
  ...
  namespace detail {
    // This exists only to maintain some sanity in figuring out overload
    // resolution for as_input_range().
    ...
    template <class T>
    using input_category_of_t =
        typename input_category_of<remove_cvref_t<T>>::type;

    template <class T>
    constexpr decltype(auto) as_input_range_impl(input_category::input_range,
                                                 T&& range) noexcept {
      return std::forward<T>(range);
    }

    template <class T>
    constexpr auto as_input_range_impl(input_category::sink,
                                       T&& input) noexcept {
      static_assert(!std::is_lvalue_reference_v<T>,
                    "Please only convert sinks to input ranges with rvalue "
                    "references (for efficiency).");
      using RX_NAMESPACE::sink;
      // Store the output in a temporary vector.
      using output_type
          = remove_cvref_t<typename remove_cvref_t<T>::output_type>;
    ...
    template <class T>
    constexpr auto as_input_range_impl(input_category::other,
  ...
  /*!
      ...
      Some combinators expect an idempotent input to avoid internal storage
     unless it is actually needed. This is a convenience wrapper for those
     combinators, which wraps a non-idempotent combinator.

      Idempotency is achieved by storing the "current" element in internal,
     temporary storage. For this reason, the output type of `R` must be
     copy-constructible.
  ...
  */
  ...
  struct idempotent_range {
    static_assert(
        !is_idempotent_v<R>);  // Don't wrap an already idempotent range.
  ...
  constexpr auto as_idempotent_input_range(R&& range) {
    // Convert the input to an input range, and then wrap it in idempotent_range
    // if it isn't already idempotent.
  ...
  // Get the equivalent range type of some type. This is the return type of
  // `as_input_range()` without cvref-qualifiers when called with T as the
  // argument (which may be any lvalue or rvalue type). The reason that we can't
  // juse use remove_cvref_t<T> is that some inputs may be converted to a
  // different type by `as_input_range()`, such as standard containers.
  template <class T>
  using get_range_type_t
      = remove_cvref_t<decltype(as_input_range(std::declval<T>()))>;

  // This function is analogous to get_range_type_t. Instead the result of
  // as_input_range() it tells you the result of as_idempotent_input_range().
  template <class T>
  using get_idempotent_range_type_t
      = remove_cvref_t<decltype(as_idempotent_input_range(std::declval<T>()))>;

  // Get the output type of T as if it was converted to a range. This is the
  // output type of the input range after conversion through `as_input_range()`.
  template <class T>
  ...
  /*!
      ...
      Note: The function does not have to be idempotent, but subsequent chained
     combinators may cache the return value if they need idempotency.
  ...
  */
  ...
  /*!
      ...
      Create an infinite sequence of T, advancing each iteration by a value of
     type U.

      T is expected to be initializable by an integral constant, and to have
     well-defined `operator+=(U)`.
  */
  ...
  struct seq {
    ...
    constexpr explicit seq(T init = T{}, U step = U{} + 1)
        : current(init), step(step) {}
  ...
  */
  ...
  /*!
      ...
      Each output is produced by passing the output of the inner range to the
     function F.
  */
  template <class F>
  struct transform {
    ...
    struct Range {
      ...
      static constexpr bool is_idempotent
          = false;  // `func` is called each time `get` is called
    ...
    // Since F may be either cheap or expensive to copy, provide overloads for
    // both const-refs and rvalue-refs.

    template <class InputRange>
    [[nodiscard]] constexpr auto
    operator()(InputRange&& input) const& noexcept {
  ...
  /*!
      ...
      If the predicate returns true, the element will be included in the output.
  ...
  */
  ...
  struct filter {
    ...
    struct Range {
      ...
      constexpr void next() noexcept {
        RX_ASSERT(!at_end());
        // XXX: For some reason, compilers are not able to merge this loop
        // condition with conditions on the input range, causing many more
        // branches than necessary. (Tested with MSVC, Clang, GCC.)
        //
        // This means that filter() can be much much slower (4x with Clang/GCC,
        // 10x with MSVC) in tight loops.
        input_.next();
    ...
    [[nodiscard]] constexpr auto operator()(InputRange&& input) const& {
      ...
      return Range<Inner>{
          as_idempotent_input_range(std::forward<InputRange>(input)), pred};
    }
    template <class InputRange>
    [[nodiscard]] constexpr auto operator()(InputRange&& input) && {
      using Inner = get_idempotent_range_type_t<InputRange>;
      return Range<Inner>{
          as_idempotent_input_range(std::forward<InputRange>(input)),
          std::move(pred)};
    }
  };
  template <class F>
  ...
  /*!
      @brief Create a range that produces at most N elements from the beginning
     of its input.
  */
  struct take {
    const size_t n;
    ...
    struct Range {
      ...
      constexpr Range(R inner, size_t n) noexcept
          : inner(std::move(inner)), n(n) {}
  ...
  */
  ...
  /*!
      ...
      Note: The element for which the predicate returned true is not included in
     the output.

      @tparam P A predicate function callable with elements of the input as its
  ...
  */
  ...
  struct until {
    ...
    struct Range {
      ...
      static constexpr bool is_finite = true;
      static constexpr bool is_idempotent
          = false;  // get() is called twice per iteration
  ...
  struct ZipRange {
    ...
    using output_type
        = std::tuple<remove_cvref_t<typename Inputs::output_type>...>;
   ...
   private:
    ...
    [[nodiscard]] constexpr output_type
    _get(std::index_sequence<Index...>) const noexcept {
      return output_type(
          std::forward_as_tuple(std::get<Index>(inputs).get()...));
    }
    ...
    _at_end(std::index_sequence<Index...>) const noexcept {
    ...
    template <size_t... Index>
    constexpr size_t _advance_by(std::index_sequence<Index...>,
                                 size_t n) noexcept {
      using RX_NAMESPACE::advance_by;
  ...
  /*!
      ...
      Until none of the ranges are at end, produce a tuple of an element from
     each range.

      The ranges are not required to produce the same number of elements, but
     elements will only be produced until one of the ranges reaches its end.
  */
  template <class... Inputs>
  [[nodiscard]] constexpr auto zip(Inputs&&... inputs) noexcept {
    // For some reason, argument deduction doesn't work here.
    return ZipRange(
        std::forward_as_tuple(as_input_range(std::forward<Inputs>(inputs))...));
  }
  ...
  /*!
      ...
      The inputs are not required to produce the same number of elements, but
     elements will only be produced until one of the ranges reaches its end.
  */
  template <class... Inputs>
  [[nodiscard]] constexpr auto enumerate(Inputs&&... inputs) noexcept {
    return ZipRange(std::forward_as_tuple(
        seq<size_t>(), as_input_range(std::forward<Inputs>(inputs))...));
  }
  ...
  /*!
      ...
      If the input produces a number of inputs that is not divisible by N,
     elements at the end will be skipped.
  */
  struct in_groups_of_exactly {
    ...
    struct Range {
      ...
      static constexpr bool is_finite = is_finite_v<R>;
      static constexpr bool is_idempotent
          = true;  // ... but we output potentially non-idempotent
                   // ranges.
      ...
      constexpr explicit Range(R inner, size_t n)
          : inner(std::move(inner)), n(n) {
        next();
      }
  ...
  /*!
      ...
      The output type is a range producing at most N elements. If the input
     produces a number of inputs that is not divisible by N, the last group
     produces will have a size < N.
  */
  struct in_groups_of {
    size_t n;
  ...
  /*!
      @brief Produce consecutive groups of elements for which P returns the same
     value, according to the Compare function.

      The output type is a range of the same types as the input. The size of the
     range is indeterminate, but always greater than 0.

      @tparam P The discriminant function. When the return value of this
     function changes, according to Compare, a new group is emitted.
      @tparam Compare The compare function used to compare return values of P.
      @tparam group_size_hint A hint about the average group size. It is 8 by
     default (somewhat arbitrarily). If smaller or larger groups are expected,
     this parameter can be tuned to avoid either preallocating too little or too
     much, respectively, when outputting to a sink that supports `reserve()`.
  */
  template <class P,
            class Compare          = std::equal_to<void>,
            size_t group_size_hint = 8>
  struct group_adjacent_by : private Compare {
    ...
    struct Range : private Compare {
      ...
      [[nodiscard]] size_t size_hint() const noexcept {
        // It's impossible to say how many groups we will produce, but we can
        // take a guess. Somewhat arbitrarily, we just assume that we will
        // produce groups of about 8.
      ...
      constexpr void fill_group() {
        auto        copy = as_input_range(inner);
        const auto& current
            = inner.get();  // lifetime extension for value types
        auto           p   = pred(current);
        size_t         n   = 0;
  ...
  group_adjacent_by(P&&, Compare&&)
      -> group_adjacent_by<remove_cvref_t<P>, remove_cvref_t<Compare>>;

  /*!
      @brief Sink element(s) into an optional.

      If the input produces 1 or more output, the last element produced will be
  ...
  */
  ...
  /*!
      ...
      The result is an optional containing the first element, or none if the
     range did not produce any outputs.
  */
  struct first {
    template <class R>
    [[nodiscard]] constexpr auto operator()(R&& input) const {
  ...
  */
  ...
  /*!
      ...
      The input range is expected to produce values conforming to the "tuple"
     concept, i.e., std::get<0>(value) and std::get<1>(value) are defined, as
     well as std::tuple_element<0> and std::tuple_element<1>.

      The first element of the output will be the key, and the second element
     will be the value.
  */
  struct to_map {
    template <class R>
    [[nodiscard]] constexpr auto operator()(R&& range) const {
      ...
      // Note: Sink only modifies the input if it actually is an rvalue
      // reference.
  ...
  */
  ...
  /*!
      ...
      Produces a single value created from consecutive calls to the provided
     function.
  ...
  */
  ...
  /*!
      ...
      The output type of the input range is compared using an instance of
     Compare, which is `std::less<void>` by default.
  ...
  */
  ...
  struct max : private Compare {
    ...
    [[nodiscard]] constexpr auto operator()(R&& range) const noexcept {
      ...
      if (RX_LIKELY(!input.at_end())) {
        ...
        // GCC 9.1 mistakenly thinks the return value is uninitialized unless we
        // declare it as a local first.
  ...
  /*!
      @brief Checks if the predicate returns true for any element in the input
     range.

      @tparam P An object callable with the result of the input range, returning
     a boolean.
  */
  template <class P>
  struct any_of {
  ...
  /*!
      @brief Checks if the predicate returns true for all elements in the input
     range.
  ...
  */
  ...
  /*!
      @brief Checks that the predicate returns false for all elements in the
     input range.
  ...
  */
  ...
  /*!
      ...
      This is usually redundant, since normal range-based for loops are
     supported, but in some cases it can yield more readable code, especially
     for long chains of combinators.
  */
  template <class F>
  struct for_each {
    F func;
  ...
  */
  ...
  struct append {
    ...
    constexpr C& operator()(R&& range) {
      // Note: sink() only advances the input range if it is actually an rvalue
      // reference.
  ...
  /// Sorting sink.
  ///
  /// Writes the result of the inner range to the output, and sorts the output
  /// container.
  template <class Compare = std::less<void>>
  struct sort : private Compare {
    ...
    struct Range : private Compare {
      ...
      constexpr void sink(Out& out) && noexcept {
        ...
        // Note: This indirection is only required because GNU libstdc++
        // implements sort() in a way that requires the predicate to be
        // copy-constructible.
        using compare_type = remove_cvref_t<output_type>;
        struct indirection {
          const Compare* cmp;
          constexpr explicit indirection(const Compare* cmp) : cmp(cmp) {}
          constexpr bool operator()(const compare_type& lhs,
                                    const compare_type& rhs) const noexcept {
            return (*cmp)(lhs, rhs);
          }
        };
    ...
    [[nodiscard]] constexpr auto
    operator()(InputRange&& input) const& noexcept {
      // Note: We don't want to use as_input_range(), because it would copy the
      // input when chaining sinks.
  ...
  ///
  /// Writes the result of the inner range to the output, calls std::unique() on
  /// the output container, and erases the remaining elements from the output.
  ///
  /// Note: std::unique() only eliminates contiguous sequences of equal
  /// elements. If all duplicates should be removed, sort the range first.
  ...
  /// Reverse range sink.
  ///
  /// Writes the result of the inner range to the output, then reverses the
  /// order of the elements using `std::reverse()`.
  ///
  /// Note: Contrary to other range implementations, this requires storage. If
  /// the output is sinked into a container, the storage for that container will
  /// be used, but otherwise temporary storage will be allocated.
  ///
  /// If multiple sinks are chained, they will reuse the same storage (i.e.,
  /// `sort() | reverse()` will operate on the same output container, modifying
  /// it in turn).
  struct reverse {
  ...
  struct ChainRange {
    ...
    constexpr explicit ChainRange(std::tuple<Rs...> inputs)
   ...
   private:
    [[noreturn]] constexpr output_type _get(std::index_sequence<>) const {
    ...
    constexpr size_t
    _size_hint(std::index_sequence<indices...>) const noexcept {
    ...
    template <size_t i, size_t... rest>
    constexpr size_t _advance_by(std::index_sequence<i, rest...>,
                                 size_t by) noexcept {
      using RX_NAMESPACE::advance_by;
      if (i == current_input) {
        auto&  input     = std::get<i>(inputs);
        ...
        if (input.at_end()) {
          ++current_input;
          return advanced
                 + _advance_by(std::index_sequence<rest...>{}, remainder);
        } else {
          return advanced;
        }
      } else {
  ...
  ///
  /// Once the first range is exhausted, values from the second range are
  /// returned, and so on.
  template <class First, class... InputRanges>
  [[nodiscard]] constexpr auto chain(First&& first,
                                     InputRanges&&... inputs) noexcept {
    if constexpr (sizeof...(InputRanges) == 0) {
      return as_input_range(std::forward<First>(first));
    } else {
      return ChainRange<get_range_type_t<First>,
                        get_range_type_t<InputRanges>...>{std::forward_as_tuple(
          as_input_range(std::forward<First>(first)),
  ...
  ///
  /// If the input is empty, the output is empty, too. Note that passing an
  /// inifite range is supported, but will never cycle.
  struct cycle {
    template <class R>
    struct Range {
      using output_type                   = typename R::output_type;
      ...
      const R prototype;
      RX_OPTIONAL<R>
          input;  // rationale: most ranges in this library are not assignable
      ...
      // Note: advance_by() not specialized because we need to know when to
      // wrap.
    };
  ...
  /// Yield an infinite list of constant values once the input range is
  /// exhausted.
  template <class V>
  struct padded {
    V value;
    ...
    struct Range {
      ...
      using output_type
          = std::common_type_t<remove_cvref_t<get_output_type_of_t<R>>, V>;
      static constexpr bool is_finite     = false;
      static constexpr bool is_idempotent = is_idempotent_v<R>;
  ...
  struct ZipLongestRange {
    ...
    using output_type = std::tuple<
        RX_OPTIONAL<remove_cvref_t<typename Inputs::output_type>>...>;
    static constexpr bool is_finite     = (is_finite_v<Inputs> && ...);
    static constexpr bool is_idempotent = (is_idempotent_v<Inputs> && ...);

    constexpr explicit ZipLongestRange(std::tuple<Inputs...> tuple)
        : inputs(std::move(tuple)) {}
   ...
   private:
    ...
    _get(std::index_sequence<Index...>) const noexcept {
      return output_type(
          std::forward_as_tuple((std::get<Index>(inputs) | first())...));
    }

    template <size_t... Index>
    constexpr void _next(std::index_sequence<Index...>) noexcept {
      ((std::get<Index>(inputs).at_end() ? 0
                                         : (std::get<Index>(inputs).next(), 0)),
       ...);
    }
  ...
  template <class... Inputs>
  ZipLongestRange(std::tuple<Inputs...>)
      -> ZipLongestRange<remove_cvref_t<Inputs>...>;

  /*!
      @brief Zip two or more ranges, return longest range.
      Until all of the ranges are at end, produce a tuple of an element from
     each range. The ranges are not required to produce the same number of
     elements, and elements will be produced until all of the ranges reach their
     end. Ranges that ended earlier produce nullopt.
  */
  template <class... Inputs>
  [[nodiscard]] constexpr auto zip_longest(Inputs&&... inputs) noexcept {
    return ZipLongestRange(
        std::forward_as_tuple(as_input_range(std::forward<Inputs>(inputs))...));
  }
  ...
  struct tee {
    ...
    [[nodiscard]] constexpr auto operator()(InputRange&& input) const noexcept {
      using Inner = get_idempotent_range_type_t<InputRange>;
      return Range<Inner>{
          as_idempotent_input_range(std::forward<InputRange>(input)), dest};
    }
  };
  template <class Dest>
  ...
  ///
  /// Supply a template parameter bigger than 1 to flatten multiple levels at
  /// once.
    ...
    struct Range {
      ...
      [[nodiscard]] constexpr size_t size_hint() const noexcept {
        ...
        if (s == std::numeric_limits<size_t>::max()) {
          ...
          // Only because this inner range is empty, does not mean all inner
          // ranges are. But either way, we have to assume the other ones have
  ...
  struct flatten {
    template <class InputRange>
    [[nodiscard]] constexpr auto operator()(InputRange&& input) const noexcept {
      return std::forward<InputRange>(input) | flatten<1>()
             | flatten<Depth - 1>();
    }
  };
  ...
  ///
  /// Use as `append(null_sink())` if you need the range to run, but you don't
  /// need the result.
  struct null_sink {
    struct value_type {};
